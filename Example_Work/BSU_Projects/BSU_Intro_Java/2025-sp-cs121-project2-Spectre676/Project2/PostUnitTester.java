...
public class PostUnitTester {
	...
	private static void testToString() {
		...
		try {
			...
			if (lines.length != 7) {
				testResults += subTestFailure(subtest, "Lines returned: 7", "Lines returned: " + lines.length);
				testPassed = false;
			} else {
				testResults += subTestPass(subtest);
			}
			...
			// Post header
			subtest = testName + " - first line has header \"Post:\"";
			boolean subtestResult = true;
			if (!lines[0].contains("Post:")) {
				testResults += subTestFailure(subtest, "First line header \"Post:\"", lines[0]);
				testPassed = false;
				subtestResult = false;
			}
			
			// Post line
			subtest = testName + " - second line contains well-formmated whitespace-separated post values";
			...
			// Comments header
			subtest = testName + " - third line has header \"Comments:\"";
			subtestResult = true;
			if (!lines[2].contains("Comments:")) {
				testResults += subTestFailure(subtest, "Third line header \"Comments:\"", lines[2]);
				testPassed = false;
				subtestResult = false;
			}

			// first comment
			subtest = testName + " - fourth line matches first comment";
			...
			// second comment
			subtest = testName + " - fifth line matches second comment";
			...
			// third comment
			subtest = testName + " - sixth line matches third comment";
			tokens = lines[5].trim().split("\\s+");
			subtestResult = true;
			try {
				@SuppressWarnings("unused")
				Instant timestamp = Instant.parse(tokens[0]);
			...
			// fourth comment
			subtest = testName + " - seventh line matches fourth comment";
			tokens = lines[6].trim().split("\\s+");
			subtestResult = true;
			try {
				@SuppressWarnings("unused")
				Instant timestamp = Instant.parse(tokens[0]);
